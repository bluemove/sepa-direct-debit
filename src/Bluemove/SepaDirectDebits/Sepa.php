...
class Sepa
{
    ...
    private function createGroupHeader()
    {
        ...
        $InitgPtyNodeId = $this->xml->createElement("Id");
        $InitgPtyNodeOrgId = $this->xml->createElement("OrgId");
        $InitgPtyNodeOthr = $this->xml->createElement("Othr");
        $InitgPtyNodeOthrId = $this->xml->createElement("Id");
        $InitgPtyNodeOthrSchmeNm = $this->xml->createElement("SchmeNm");
        $InitgPtyNodeOthrSchmeCd = $this->xml->createElement("Cd");


        //Set the values for the nodes
        $MsgIdNode->nodeValue = $this->makeMsgId();
        $CreDtTmNode->nodeValue = date("Y-m-d\TH:i:s");
        $InitgPtyNodeOthrSchmeCd->nodeValue = 'CORE';

        //If using lower than PHP 5.4.0, there is no ENT_XML1
        if (version_compare(PHP_VERSION, '5.4.0') >= 0) {
            $NmNode->nodeValue = htmlentities($this->config['name'], ENT_XML1, 'UTF-8');
            $InitgPtyNodeOthrId->nodeValue = htmlentities($this->config['creditor_id'], ENT_XML1, 'UTF-8');
        } else {
            $NmNode->nodeValue = htmlentities($this->config['name'], ENT_QUOTES, 'UTF-8');
            $InitgPtyNodeOthrId->nodeValue = htmlentities($this->config['creditor_id'], ENT_QUOTES, 'UTF-8');
        }

        //InitgPtyNode
        $InitgPtyNodeOthr->appendChild($InitgPtyNodeOthrId);
        $InitgPtyNodeOthr->appendChild($InitgPtyNodeOthrSchmeNm);
        $InitgPtyNodeOthrSchmeNm->appendChild($InitgPtyNodeOthrSchmeCd);
        $InitgPtyNodeOrgId->appendChild($InitgPtyNodeOthr);
        $InitgPtyNodeId->appendChild($InitgPtyNodeOrgId);

        //Append the nodes
        $InitgPtyNode->appendChild($NmNode);
        $InitgPtyNode->appendChild($InitgPtyNodeId);
